...
def test_iris():
    ...
    theta_optimizer_varrreg = optimizer.optimize(Z, w=np.ones(y.shape), varreg_lambda=0.1).x
...
def test_iris_kyfan():
    ...
    theta_optimizer_varreg = optimizer.optimize(
        Z, w=np.ones(y.shape), block_size=10, k=10, max_len=y.shape[0], varreg_lambda=0.1
    ).x
